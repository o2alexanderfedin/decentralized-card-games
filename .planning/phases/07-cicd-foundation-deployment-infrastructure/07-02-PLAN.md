...
must_haves:
  truths:
    - "Merge to main triggers automatic deployment to GitHub Pages (production)"
    - "Merge to develop triggers automatic deployment to GitHub Pages (staging)"
  ...
  artifacts:
    - path: ".github/workflows/deploy.yml"
      provides: "Deployment workflow configuration with production and staging environments"
  ...
  key_links:
    ...
    - from: ".github/workflows/deploy.yml"
      to: "GitHub deployment environments"
      via: "environment context"
      pattern: "github-pages|staging"
---

<objective>
Create the deployment workflow that builds and deploys the composite site to GitHub Pages when code is merged to main (production) or develop (staging preview).

Purpose: Enable automatic deployment so the documentation site stays current with the codebase, with staging preview for pre-release validation
...
<task type="auto">
  <name>Task 2: Create deployment workflow with production and staging environments</name>
  ...
  <action>
    ...
    **Triggers:**
    ...
    - push to: develop (staging preview deployment)
    ...
    **Jobs:**
    ...
    2. `deploy-production` (name: "Deploy to Production")
    ...
    3. `deploy-staging` (name: "Deploy to Staging")
    ...
    **Important:** GitHub Pages only supports one deployment per repository, so both staging and production deploy to the same URL. The environment distinction provides:
    - Separate deployment history tracking in GitHub UI
    - Ability to add environment-specific protection rules later
    - Clear audit trail of which branch triggered deployment

    Per CONTEXT.md decision: "Preview deployment: Deploy develop branch to staging URL for pre-release preview"
  ...
  <verify>
    ...
    Triggers on both main and develop: `grep -E "main|develop" .github/workflows/deploy.yml`
    Has conditional deploy jobs: `grep -E "refs/heads/main|refs/heads/develop" .github/workflows/deploy.yml`
  </verify>
  <done>
    deploy.yml exists with build and conditional deploy jobs
    Workflow triggers on push to main (production) and develop (staging)
    ...
    Production deploys to github-pages environment
...
3. Deploy workflow triggers on both main and develop branch pushes
...
7. Conditional deployment jobs exist for production (main) and staging (develop)
</verification>

<success_criteria>
- Deployment workflow ready to deploy to GitHub Pages on merge to main (production)
- Deployment workflow ready to deploy to GitHub Pages on merge to develop (staging preview)
...
- Environment tracking distinguishes production vs staging deployments
</success_criteria>
