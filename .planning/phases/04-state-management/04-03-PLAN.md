...
- Build the StateBackend object with `useMemo`:
  ```
  { getState: () => stateRef.current, dispatch, subscribe: (listener) => { listeners.add(listener); return () => listeners.delete(listener); } }
  ```
...
- dispatch from useReducer is already stable, so no wrapping needed.
...
2. **Dispatch moveCard:** Setup initial state with cards in 'deck'. Use `useGameActions` to dispatch a MOVE_CARD action. Verify `useLocation('hand')` reflects the moved card.

3. **dealStandardDeck integration:** Dispatch `dealStandardDeck()` action via `useGameActions`. Verify `useLocation('deck')` returns 52 cards.
