...
must_haves:
  truths:
    ...
    - "useGameActions returns a dispatch function with Redux-style API: dispatch('MOVE_CARD', { cardIndex, from, to })"
  ...
  artifacts:
    ...
    - path: "src/hooks/useGameActions.ts"
      provides: "Hook returning Redux-style dispatch(type, payload) function"
...
1. Define the dispatch type alias matching user's Redux-style API decision:
   ```typescript
   export type GameDispatchFn = (type: string, payload?: Record<string, unknown>) => void;
   ```

2. Define `StateBackend` interface:
   ```typescript
   ...
   ```
   IMPORTANT: `dispatch` accepts `(type: string, payload?)` NOT `(action: GameAction)`. This matches the user decision: `dispatch('MOVE_CARD', { cardIndex: 0, from: 'deck', to: 'hand' })`.

3. Create `StateBackendContext = createContext<StateBackend | null>(null)`
4. Export `useStateBackend()` hook that reads from context and throws if null:
...
- Return type: `GameDispatchFn` -- i.e., `(type: string, payload?: Record<string, unknown>) => void`
- Usage: `const dispatch = useGameActions(); dispatch('MOVE_CARD', { cardIndex: 0, from: 'deck', to: 'hand' });`
...
- Test useGameActions returns dispatch function; calling `dispatch('MOVE_CARD', { cardIndex: 0, from: 'deck', to: 'hand' })` invokes backend.dispatch with those args
- Test all hooks throw when used outside provider

Update `src/hooks/index.ts` to export:
- useStateBackend, StateBackend, StateBackendContext, GameDispatchFn
...
- StateBackend interface defined with getState/dispatch(type, payload)/subscribe
- dispatch follows Redux-style API: dispatch('MOVE_CARD', { cardIndex, from, to })
