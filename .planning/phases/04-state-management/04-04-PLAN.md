...
- Create the StateBackend adapter that bridges Redux store to the StateBackend interface:
  ```typescript
  const backend: StateBackend = useMemo(() => ({
    getState: () => reduxStore.getState()[gameSlice.reducerPath],
    dispatch: (action: GameAction) => {
      // Map GameAction type strings to slice action creators
      // e.g., action.type === 'MOVE_CARD' -> reduxStore.dispatch(gameSlice.actions.moveCard(action.payload))
    },
    subscribe: (listener) => reduxStore.subscribe(listener),
  }), [reduxStore]);
  ```
- The dispatch adapter maps GameAction union types to slice action creators. Use a lookup object mapping action.type strings to slice action creator functions.
...
2. **Dispatch via useGameActions:** Use `useGameActions` to dispatch moveCard. Verify `useLocation` reflects change.

3. **dealStandardDeck integration:** Dispatch SET_LOCATIONS with 52 cards. Verify `useLocation('deck')` returns 52 cards.
